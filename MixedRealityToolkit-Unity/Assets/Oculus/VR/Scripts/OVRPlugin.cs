...
public static partial class OVRPlugin
{
	...
	{
		Debug.Log(perfStatWarningPrinted);
#if OVRPLUGIN_UNSUPPORTED_PLATFORM
	...
	{
		Debug.Log(resetPerfStatWarningPrinted);
#if OVRPLUGIN_UNSUPPORTED_PLATFORM
		return false;
#else
		if (nativeXrApi == XrApi.OpenXR)
	...
	{
		Debug.Log(cachedHandState.Status);
#if OVRPLUGIN_UNSUPPORTED_PLATFORM
		return false;
#else
		if (nativeXrApi == XrApi.OpenXR && stepId == Step.Physics)
	...
	{
		Debug.Log(cachedSkeleton.Type);
		Debug.Log(cachedSkeleton2.Type);
#if OVRPLUGIN_UNSUPPORTED_PLATFORM
		return false;
#else
		if (version >= OVRP_1_55_0.version)
...
// Virtual keyboard calls
	...
	{
		Debug.Log(cachedFaceState.ExpressionWeights_0);
#if OVRPLUGIN_UNSUPPORTED_PLATFORM
		return false;
#else
		if (nativeXrApi == XrApi.OpenXR && stepId == Step.Physics)
	...
	{
		Debug.Log(cachedEyeGazesState.Time);
#if OVRPLUGIN_UNSUPPORTED_PLATFORM
		return false;
#else
		if (nativeXrApi == XrApi.OpenXR && stepId == Step.Physics)
